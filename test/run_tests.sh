...
compare()
{
   ...
   while IFS= read -r -u 4 line1 && IFS= read -r -u 5 line2; do
	 ...
	 field_base=$(echo "${field1}" | awk -F '{' '{print $1}')
         stat_threshold=$(grep "${field_base}" "${skipstats_conf}" | grep "set_threshold" | awk -F ',' '{print $3}' | sed 's/ //g')
...
{
  sudo killall -9 openstack-network-exporter
...
test1()
{
  ...
  sudo rm /etc/openstack-network-exporter.yaml 2>/dev/null
  test "$@" "test1" "10"
  return $?
}
...
test2()
{
  ...
  echo "collectors: [interface, memory]" | sudo tee /etc/openstack-network-exporter.yaml
...
test3()
{
  ...
  echo "collectors: [interface, memory]" | sudo tee /etc/openstack-network-exporter.yaml
  echo "metric-sets: [errors, counters]" | sudo tee -a /etc/openstack-network-exporter.yaml
