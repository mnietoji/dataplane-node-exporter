	...
	for bridge in ${bridges};do
		...
		for port in ${ports};do
			...
			for field in "${!fields[@]}"; do
				val=$(ovs-vsctl get interface "${port}"
					"${field}")
				val="${val_encoding[${val}]}"
				cat << EOF
...
get_pmd_rxq_stats(){
	...
	while read -r line; do
		...
		pmd*)
			...
			;;
		*port*)
			rxq=$(echo "${line}" | sed -En 's/.*queue-id:[ ]{0,}([a-zA-Z0-9]+) .*/\1/p')
			interface=$(echo "${line}" | sed -En 's/.*port:[ ]{0,}([a-zA-Z0-9]+) .*/\1/p')
			usage=$(echo "${line}" | sed -En 's/.*usage:[ ]{0,}([a-zA-Z0-9]+) .*/\1/p')
			enabled=$(echo "${line}" | sed -En 's/.*\(([a-zA-Z0-9]+)\).*/\1/p')
			[[ "${enabled}" != "enabled" ]] && enabled=1 || enabled=0
			cat << EOF
ovs_pmd_rxq_enabled{cpu="${core_id}",interface="${interface}",numa="${numa_id}",rxq="${rxq}"} ${enabled}
ovs_pmd_rxq_usage{cpu="${core_id}",interface="${interface}",numa="${numa_id}",rxq="${rxq}"} ${usage}
EOF
			;;
		*overhead*)
			overhead=$(echo "${line}" | sed -En 's/.*overhead:[ ]{0,}([a-zA-Z0-9]+) .*/\1/p')
			cat << EOF
ovs_pmd_cpu_overhead{cpu="${core_id}",numa="${numa_id}"} ${overhead}
EOF
			;;
		*isolated*)
			isolated=$(echo "${line}" | sed -En 's/.*isolated:[ ]{0,}([a-zA-Z0-9]+) .*/\1/p')
	...
	ovs_pid=$(cat /run/openvswitch/ovs-vswitchd.pid)
        for status in /proc/$ovs_pid/task/*/status; do
                while read -r line; do
                        case "$line" in
                        Name:*pmd*)
                                 cpu=$(echo $line | sed -En 's/.*pmd-c([0-9]+).*/\1/')
                                 numa=${core_list_map[$cpu]}
                                 ;;
                        voluntary_ctxt_switches:*)
                                 value=$(echo $line | sed -En 's/.*([0-9]+).*/\1/p')
                                 cat <<EOF
