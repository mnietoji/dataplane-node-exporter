...
get_coverage_stats() {
	ovs-appctl  coverage/show | awk '/total/
		{ print "ovs_coverage_"$1"_total",$6 }'
}
...
get_pmd_rxq_stats() {
	...
	while read -r line; do
		...
		*port*)
			...
			interface=$(echo "${line}" | sed -En 's/.*port: *([0-9]+).*/\1/p')
